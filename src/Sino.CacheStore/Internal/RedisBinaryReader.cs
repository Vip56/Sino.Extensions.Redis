...
{
    public class RedisBinaryReader
    {
        readonly Stream _stream;
        readonly Encoding _encoding;

        public RedisBinaryReader(Stream stream, Encoding encoding)
        {
            _stream = stream;
            _encoding = encoding;
        }
        ...
        {
            var type = (RedisMessage)_stream.ReadByte();
            if (type == RedisMessage.Error)
                throw new CacheStoreException(ReadStatus(false));
            return type;
        }
        ...
        public void ExpectType(RedisMessage expectedType)
        {
            ...
            if (type != expectedType)
                throw new CacheStoreProtocolException($"Unexpected response type:{type} (expecting {expectedType})");
        }
        ...
        public void ExpectSize(long expectedSize, long actualSize)
        {
            if (actualSize != expectedSize)
                throw new CacheStoreProtocolException($"Expected {expectedSize} elements: got {actualSize}");
        }
        ...
        public void ReadCRLF()
        {
            ...
            if (r != (byte)13 && n != (byte)10)
                throw new CacheStoreProtocolException($"Expecting CRLF: got bytes: {r}, {n}");
        }
        ...
        public object Read(bool bulkAsString = false)
        {
            ...
            switch (type)
            {
                ...
                case RedisMessage.Error:
                    throw new CacheStoreException(ReadStatus(false));
                default:
                    throw new CacheStoreProtocolException($"Unsupported response type: {type}");
            }
        }

        public void ExpectBytesRead(long expecting, long actual)
        {
            if (actual != expecting)
                throw new CacheStoreProtocolException($"Expecting {expecting} bytes: got {actual} bytes.");
        }
    }
}
