...
### 基准测试  
为了保证大家的使用，针对SDK进行了Set与Get在单线程与多线程下的测试，便于大家选择：   

单线程同步调用下Set/Get 5000条数据耗时：
1. 5434ms/4794ms
2. 5018ms/4620ms
3. 5173ms/4733ms
4. 4996ms/4932ms
...
其中多线程异步调用由于采用的是Parallel故实际操作并没有等待结果返回所以耗时较短。    
